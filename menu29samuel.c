...
case 1:
	...
	while (fscanf(puntero, "%d %s %f %i %s", &vector[i].identificador, vector[i].usuario, &vector[i].saldo, &vector[i].telefono, &vector[i].numCuenta)!=EOF) {
		printf("%d Nombre:%s \tSaldo actual:%.2f$\t Tlfno:%d\t NCuenta:%s\n", vector[i].identificador, vector[i].usuario, vector[i].saldo, vector[i].telefono, vector[i].numCuenta);
		i++;
	}
	fclose(puntero);
	...
	do{
		...
		printf("Indique su numero de cuenta\n", i);
		scanf("%s", &vector[i].numCuenta);
	...
	for(j=0;j<i;j++){
		fprintf(puntero,"%d\t%s\t%f\t%i\t%s\n", vector[j].identificador, vector[j].usuario, vector[j].saldo, vector[j].telefono, vector[j].numCuenta);
		}
